...
Reprenons l'exemple présenté lundi de la génération d'une variable indicatrice de la Métropole en traitant les codes département comme des nombres. 
...
Lors la présentation de lundi  
nous avons vu que plusieurs type de communes existent :
...
Du fait qu'il y a plusieurs façon de délimiter les communes, voyons si les différences proviennent de là :
...
on retrouve de clefs manquantes dans epci2021.comp. Les ( 2777 - 2157 ) = 620 lignes encore manquantes s'expliquent par la répétition 
du code commune pour certaines communes déléguées qui ont repris le code de la commune avec laquelle elles ont été fusionnée.
...
dans une plage prédéterminée pour s'en servir d'indexe. L'exemple de fonction utilisé est le suivant :
...
Cette fonction de hachage est un classique du genre. Elle est notamment connue pour apparaître dans le _Red Dragon Book_
qui est incunable de la littérature sur la conception des compilateurs. 
...
(plus loin). Et, comme ils le disent, "This is not the best possible algorithm, but it has the merit of extreme simplicity". 
...
le débordement par le haut qui est normal ici conduirait à la production de NA ce qui n'est clairement pas ce qu'on veut.
...
En conséquence de quoi, L'Abergement-Clémenciat à la même index que Gueugnon
...
Pour réduire le nombre de clefs ayant le même index, on peut commencer par doubler la capacité de la table, 
